  ...
  const renderChannelButton = (cnl: StreamChannel) => {
    ...
    return (
      <button
        ...
        className={cn(
          "flex items-center gap-2 w-full px-2 py-1.5 rounded-md dark:hover:bg-muted hover:bg-gray-300",
          cnl.id === activeChannel?.id && "bg-gray-300 dark:bg-muted"
        )}
      >
        <div className="flex-shrink-0 w-[50px] h-[50px]">
          <BetterImage
            src={
              cnl?.id === user.id ? admin.imageUrl : (cnl.data?.image as string)
            }
            width={250}
            height={250}
