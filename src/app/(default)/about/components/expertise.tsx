...
const ExpertiseSection = memo(() => {
  ...
  useEffect(() => {
    const handleWindowResize = () => {
      ...
      const calculatedColumns = Math.floor(adjustedWidth / 129);
        ...
        >
          <div className="flex justify-center items-center gap-6">
            {[...Array(columnsPerRow)].map((_, index) => (
              <div
                key={index}
                className="relative flex justify-center items-center gap-2.5 w-[105px] aspect-square bg-gradient-to-b from-transparent to-background/50 backdrop-blur z-10 transition-all duration-300 hover:scale-105"
              >
                <div className="absolute bottom-0 left-0 w-full h-[1px] bg-border" />
                <div className="absolute left-0 top-0 w-[1px] h-full bg-gradient-to-b from-transparent to-border" />
                <div className="absolute right-0 top-0 w-[1px] h-full bg-gradient-to-b from-transparent to-border" />
              </div>
            ))}
          </div>
                ...
                >
                  <div className="flex justify-center items-center gap-6">
                    {[
                      ...
                      <div
                        key={`left-${index}`}
                        className="flex justify-center items-center gap-2.5 w-[105px] aspect-square bg-background/50 backdrop-blur shadow-lg hover:shadow-xl z-10 transition-all duration-300 border hover:scale-105"
                      />
                    ))}
                    {rowItems.map((technology, itemIndex) => (
                      <motion.div
                        key={technology.name}
          ...
          <div className="flex justify-center items-center gap-6">
            {[...Array(columnsPerRow)].map((_, index) => (
              <div
                key={index}
                className="relative flex justify-center items-center gap-2.5 w-[105px] aspect-square bg-gradient-to-t from-transparent to-background/50 backdrop-blur z-10 transition-all duration-300 hover:scale-105"
              >
                <div className="absolute top-0 left-0 w-full h-[1px] bg-border" />
                <div className="absolute left-0 top-0 w-[1px] h-full bg-gradient-to-t from-transparent to-border" />
                <div className="absolute right-0 top-0 w-[1px] h-full bg-gradient-to-t from-transparent to-border" />
              </div>
            ))}
          </div>
        </motion.div>
